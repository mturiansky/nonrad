...
class SommerfeldTest(unittest.TestCase):
    ...
    def test_neutral(self):
        self.assertAlmostEqual(sommerfeld_parameter(**self.args), 1.0)
        self.args["method"] = "Analytic"
        self.assertAlmostEqual(sommerfeld_parameter(**self.args), 1.0)

    def test_attractive(self):
        self.args["Z"] = -1
        self.assertGreater(sommerfeld_parameter(**self.args), 1.0)
        self.args["method"] = "Analytic"
        self.assertGreater(sommerfeld_parameter(**self.args), 1.0)

    def test_repulsive(self):
        self.args["Z"] = 1
        self.assertLess(sommerfeld_parameter(**self.args), 1.0)
        self.args["method"] = "Analytic"
        self.assertLess(sommerfeld_parameter(**self.args), 1.0)

    def test_list(self):
        self.args["T"] = np.linspace(0.1, 1000, 100)
        self.assertEqual(sommerfeld_parameter(**self.args), 1.0)
        self.args["Z"] = -1
        self.assertTrue(np.all(sommerfeld_parameter(**self.args) > 1.0))
        self.args["Z"] = 1
        self.assertTrue(np.all(sommerfeld_parameter(**self.args) < 1.0))
        self.args["Z"] = 0
        self.args["method"] = "Analytic"
    ...
    def test_sommerfeld_dim(self):
        ...
        self.assertAlmostEqual(sommerfeld_parameter(**self.args), 2.0, places=2)
        self.args["method"] = "a"
        self.assertAlmostEqual(sommerfeld_parameter(**self.args), 2.0, places=5)
        self.args["method"] = "i"
        self.args["Z"] = 1
        self.assertLess(sommerfeld_parameter(**self.args), 1.0)
        self.args["method"] = "a"
...
class ChargedSupercellScalingTest(unittest.TestCase):
    def test_find_charge_center(self):
        ...
        self.assertTrue(np.allclose(find_charge_center(density, lattice), [0.49] * 3))
        density = np.zeros((50, 50, 50))
        density[0, 0, 0] = 1.0
        self.assertTrue(np.allclose(find_charge_center(density, lattice), [0.0] * 3))
    ...
    def test_radial_distribution(self):
        ...
        self.assertAlmostEqual(r[np.where(n == 1.0)[0][0]], dist)
