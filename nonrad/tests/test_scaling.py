...
def _old_sommerfeld_parameter(
    T: float | np.ndarray, Z: int, m_eff: float, eps0: float, method: str = "Integrate"
) -> float | np.ndarray:
    if Z == 0:
        return 1.0
...
class SommerfeldTest(unittest.TestCase):
    def setUp(self):
        self.args = {"T": 300, "Z": 0, "m_eff": 1.0, "eps0": 1.0, "method": "Integrate"}

    def test_neutral(self):
        assert round(abs(sommerfeld_parameter(**self.args) - 1.0), 7) == 0
        self.args["method"] = "Analytic"
        assert round(abs(sommerfeld_parameter(**self.args) - 1.0), 7) == 0

    def test_attractive(self):
        self.args["Z"] = -1
        assert sommerfeld_parameter(**self.args) > 1.0
        self.args["method"] = "Analytic"
        assert sommerfeld_parameter(**self.args) > 1.0
    ...
    def test_list(self):
        ...
        assert sommerfeld_parameter(**self.args) == 1.0
        self.args["Z"] = -1
        assert np.all(sommerfeld_parameter(**self.args) > 1.0)
        self.args["Z"] = 1
        assert np.all(sommerfeld_parameter(**self.args) < 1.0)
        self.args["Z"] = 0
        self.args["method"] = "Analytic"
    ...
    def test_compare_methods(self):
        self.args = {"T": 100, "Z": -1, "m_eff": 0.2, "eps0": 8.9, "method": "Integrate"}

        f0 = sommerfeld_parameter(**self.args)
        self.args["method"] = "Analytic"
        f1 = sommerfeld_parameter(**self.args)
        assert round(abs(f0 - f1), 2) == 0
    ...
    def test_sommerfeld_dim(self):
        self.args = {"T": 200, "Z": -1, "m_eff": 0.2, "eps0": 8.9, "dim": 2, "method": "Integrate"}

        assert round(abs(sommerfeld_parameter(**self.args) - 2.0), 2) == 0
        self.args["method"] = "a"
        assert round(abs(sommerfeld_parameter(**self.args) - 2.0), 5) == 0
        self.args["method"] = "i"
        self.args["Z"] = 1
        assert sommerfeld_parameter(**self.args) < 1.0
        self.args["method"] = "a"
...
class ChargedSupercellScalingTest(unittest.TestCase):
    def test_find_charge_center(self):
        ...
        assert np.allclose(find_charge_center(density, lattice), [0.49] * 3)
        density = np.zeros((50, 50, 50))
        density[0, 0, 0] = 1.0
        assert np.allclose(find_charge_center(density, lattice), [0.0] * 3)

    def test_distance_PBC(self):
        a = np.array([0.25] * 3)
        b = np.array([0.5] * 3)
        lattice = np.eye(3)
        assert distance_PBC(a, b, lattice) == np.sqrt(3) * 0.25
        b = np.array([0.9] * 3)
        assert distance_PBC(a, b, lattice) == np.sqrt(3) * 0.35
    ...
    def test_radial_distribution(self):
        ...
        assert round(abs(r[np.where(n == 1.0)[0][0]] - dist), 7) == 0
    ...
    def test_charged_supercell_scaling_VASP(self):
        f = charged_supercell_scaling_VASP(str(TEST_FILES / "WAVECAR.C-"), 189, def_index=192)
        assert round(abs(f - 1.08), 7) == 0

    @pytest.mark.filterwarnings("ignore:Covariance of the parameters")
    def test_charged_supercell_scaling(self):
        # test that numbers work out for homogeneous case
        wf = np.ones((20, 20, 20))
        f = charged_supercell_scaling(wf, 10 * np.eye(3), np.array([0.0] * 3))
        assert round(abs(f - 1.00), 7) == 0

        # test the plotting stuff
        wf = np.ones((1, 1, 1))
        f = charged_supercell_scaling(wf, 10 * np.eye(3), np.array([0.0] * 3), fig=FakeFig())
        assert round(abs(f - 1.00), 7) == 0
        f = charged_supercell_scaling(
            wf, 10 * np.eye(3), np.array([0.0] * 3), fig=FakeFig(), full_range=True
        )
        assert round(abs(f - 1.00), 7) == 0
...
class ThermalVelocityTest(unittest.TestCase):
    def test_thermal_velocity(self):
        f = thermal_velocity(1.0, 1.0)
        assert round(abs(f - np.sqrt(3 * const.k / const.m_e) * 1e2), 7) == 0
        f = thermal_velocity(np.array([1.0]), 1.0)
